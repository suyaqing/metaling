function [MDP] = spm_MDP_VB_X_YS(MDP,OPTIONS)
...
for m = 1:size(MDP,1)
    ...
    for f = 1:Nf(m)
        for j = 1:Nu(m,f)
            ...
            if isfield(MDP,'b') && ~HMM
                ...
                sB{m,f}(:,:,j) = spm_norm(MDP(m).B{f}(:,:,j) );
                rB{m,f}(:,:,j) = spm_norm(MDP(m).B{f}(:,:,j)'); %each column adds to 1, evenly distributed to 1-entries in the corresponding column in sB'
            end
    ...
    for f = 1:Nf(m)
        ...
        for k = 1:Np(m)
            x{m,f}(:,1,k) = D{m,f}; % expectation after T=1 set to uniform--YS
        end
    end
            ...
            for f = 1:size(MDP(m).link,1)
                for g = 1:size(MDP(m).link,2)
                    if ~isempty(MDP(m).link{f,g})
                        ...
                        % empirical priors: top layer outcome drives bottom
                        % initial state--YS
                        %--------------------------------------------------
            ...
            for k = p{m}                % loop over plausible policies
                ...
                for i = 1:Ni            % iterate belief updates
                    ...
                    for j = 1:S         % loop over future time points
                        ...
                        %--------------------------------------------------
                        if j <= t % x set to default by previous loop--YS
                            for f = 1:Nf(m)
                                xq{m,f} = full(x{m,f}(:,j,k));
                            end
                        end
                        
                        for f = 1:Nf(m) % at each iteration, F is the sum of free energy of all policies--YS
