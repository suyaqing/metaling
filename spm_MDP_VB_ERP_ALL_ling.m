function [U, W, D] = spm_MDP_VB_ERP_ALL_ling(MDP, sen)
...
for m = 1:numel(MDP)
    [xo, xa, xv, xl, xsyn] = spm_MDP_VB_ERP_ling(MDP);
...
[ua, ~, wa] = spm_MDP_VB_ERP_align(xa, u21, MDP.D{2}, 2);
[uv, ~, wv] = spm_MDP_VB_ERP_align(xv, u21, MDP.D{3}, 2);
[ul, ~, wl] = spm_MDP_VB_ERP_align(xl, u21, MDP.D{4}, 2);
% [um, ~, wm] = spm_MDP_VB_ERP_align(xm, u21, MDP.D{6}, 2);
[usyn, ~, wsyn] = spm_MDP_VB_ERP_align(xsyn, u21, MDP.D{5}(:, 1), 2);
% [uc, ~, wc] = spm_MDP_VB_ERP_align(xc, u21, MDP.D{1}, 2);
[uo, ~, wo] = spm_MDP_VB_ERP_align(xo, u21, MDP.D{1}, 2);
...
factoro = MDP(1).label.factor{1};
nameo   = MDP(1).label.name{1};

factora = MDP(1).label.factor{2};
namea   = MDP(1).label.name{2};
 
factorv = MDP(1).label.factor{3};
namer   = MDP(1).label.name{3};

factorl = MDP(1).label.factor{4};
namep   = MDP(1).label.name{4};

% factorm = MDP(1).label.factor{6};
% namem   = MDP(1).label.name{6};

factorsyn = MDP(1).label.factor{5};
namesyn   = MDP(1).label.name{5};
...
h = subplot(5,1,1); plot_raster(h, t(idxt), Uo(idxt, :), factoro, nameo, 1, rp1);
set(gca,'XTickLabel',[])

h = subplot(5,1,2); plot_raster(h, t(idxt), Ua(idxt, :), factora, namea, 1, rp1);
set(gca,'XTickLabel',[])

h = subplot(5,1,3); plot_raster(h, t(idxt), Uv(idxt, :), factorv, namer, 1, rp1);
set(gca,'XTickLabel',[])

h = subplot(5,1,4); plot_raster(h, t(idxt), Ul(idxt, :), factorl, namep, 1, rp1);
set(gca,'XTickLabel',[])

h = subplot(5,1,5); plot_raster(h, t(idxt), Usyn(idxt, :), factorsyn, namesyn, 1, rp1);
% set(gca,'XTickLabel',[])
